...
def fetch_stock_data(ticker, period="30d", interval="60m"):
    ...
    try:
        ...
        if df.empty:
            ...
            return None
        df.index = pd.to_datetime(df.index).tz_localize(None)  # Ensure timezone consistency
        return df
    except Exception as e:
        st.error(f"Error fetching data for {ticker}: {str(e)}")
        return None

# Build aligned, forward-filled close prices for cross-asset features
def get_aligned_ffill(tickers, days_history):
    ...
    for t in tickers:
        ...
        if df is not None and not df.empty:
            all_data[t] = df
    # Use target ticker index as reference
...
def create_features(df, ticker, aligned_ffill, all_data, feature_columns):
    ...
    # PATCH: Add price column, as in notebook
...
def predict_with_models(features, current_price, scaler, cnn_model, lstm_model, xgb_model, meta_model):
    try:
        ...
        # Convert predicted next-hour return to price
        predicted_price = current_price * (1 + final_pred)

        pred_change_pct = final_pred * 100  # percentage return
...
if cnn_model and lstm_model and xgb_model and meta_model and scaler and feature_columns:
    ...
    # Fetch aligned, ffilled data for all notebook tickers
    tickers_from_notebook = ["AAPL", "MSFT", "AMZN", "GOOGL", "TSLA", "NVDA", "JPM", "JNJ", "XOM", "CAT", "BA", "META"]
